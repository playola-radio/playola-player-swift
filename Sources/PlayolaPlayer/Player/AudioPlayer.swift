...
//    public weak var delegate: AudioPlayerDelegate?
    ...
    init() {
        do {
            ...
            os_log("Error setting up session: %@", log: AudioPlayer.logger, type: .default, #function, #line, error.localizedDescription)
        }
...
//        playerNode.installTap(onBus: 0, bufferSize: TapProperties.default.bufferSize, format: TapProperties.default.format, block: onTap(_:_:))
    ...
    func play() {
        os_log("%@ - %d", log: AudioPlayer.logger, type: .default, #function, #line)
...
//            delegate?.player(self, didChangePlaybackState: true)
            ...
            os_log("Error starting engine: %@", log: AudioPlayer.logger, type: .default, #function, #line, error.localizedDescription)
        }
    }
    ...
    public func stop() {
        if !engine.isRunning {
            do {
                ...
                os_log("Error starting engine while stopping: %@", log: AudioPlayer.logger, type: .default, #function, #line, error.localizedDescription)
                return
            }
        }
        playerNode.stop()
...
//        delegate?.player(self, didChangePlaybackState: false)
    ...
    public func loadFile(with url: URL) {
        ...
        do {
            ...
            os_log("Error loading (%@): %@", log: AudioPlayer.logger, type: .error, #function, #line, url.absoluteString, error.localizedDescription)
        }
    }
