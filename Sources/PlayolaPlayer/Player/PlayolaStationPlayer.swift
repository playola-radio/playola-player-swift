...
/// ```
@MainActor
public final class PlayolaStationPlayer: ObservableObject {
  ...
  @MainActor
  init(fileDownloadManager: FileDownloadManaging? = nil) {
    self.fileDownloadManager = fileDownloadManager ?? FileDownloadManagerAsync.shared
    authProvider = nil
    listeningSessionReporter = ListeningSessionReporter(stationPlayer: self, authProvider: nil)
  ...
  private func reportDecodingError(_ decodingError: DecodingError) async -> String {
    ...
    case let .dataCorrupted(reportedContext):
  ...
  public func play(stationId: String, atDate: Date? = nil) async throws {
    ...
    scheduleOffset = atDate?.timeIntervalSinceNow

    self.stationId = stationId

    // Get the schedule
    currentSchedule = try await getUpdatedSchedule(stationId: stationId)
...
extension PlayolaStationPlayer: SpinPlayerDelegate {
  public func player(_: SpinPlayer, startedPlaying spin: Spin) {
  ...
  public nonisolated func player(
    _: SpinPlayer,
    didPlayFile _: AVAudioFile,
    atTime _: TimeInterval,
    withBuffer _: AVAudioPCMBuffer
  ) {}

  public func player(_: SpinPlayer, didChangeState state: SpinPlayer.State) {
    os_log(
