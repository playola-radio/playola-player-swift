...
class AudioBufferTestUtilities {
  ...
  static func createBuffer(samples: [Float],
                           format: AVAudioFormat? = nil) -> AVAudioPCMBuffer? {
  ...
  static func extractSamples(from buffer: AVAudioPCMBuffer,
                             channel: Int = 0) -> [Float] {
    guard let channelData = buffer.floatChannelData,
          channel < buffer.format.channelCount else {
      return []
    }
  ...
  static func generateTestSamples(pattern: SamplePattern,
                                  count: Int,
